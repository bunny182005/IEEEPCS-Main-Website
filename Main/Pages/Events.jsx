...
const Events = () => {
  ...
  // const [skipAnimation, setSkipAnimation] = useState(false); // <-- REMOVED this state
  const containerRef = useRef(null);
  const flagshipRef = useRef(null);
  const navigate = useNavigate();

  // Check if we're returning from a detail page and need to scroll to flagship
  useEffect(() => {
    const isReturningFromDetail = sessionStorage.getItem('returningFromEventDetail');

    if (isReturningFromDetail) {
      // setSkipAnimation(true); // <-- REMOVED
      sessionStorage.removeItem('returningFromEventDetail');

      // Scroll immediately to flagship section
      setTimeout(() => {
  ...
  // ... (all your data arrays and style functions remain exactly the same) ...
  ...
  const getPosterStyle = (index, column) => {
    ...
    if (index < 3) {
      ...
      if (column === 'left') {
        return { 
          opacity: 1, 
          transform: `translate(-${easedExit * 100}vw, 50vh)`, 
          pointerEvents: 'none' 
        };
      } else if (column === 'center') {
        return { 
          opacity: 1, 
          transform: `translateY(-${50 + easedExit * 100}vh)`, 
          pointerEvents: 'none' 
        };
        ...
        if (index === 3) {
          ...
          if (column === 'center') {
            ...
            return { 
              ...
              transform: `translateY(${translateY}vh)`, 
              pointerEvents: easedProgress > 0.5 ? 'auto' : 'none'
            };
          } else {
            const startY = -50;
            const translateY = startY - (startY - targetY) * easedProgress;
        ...
        if (index === 3) {
          ...
          if (column === 'left') {
            return { 
              ...
              transform: `translate(-${easedFade * 100}vw, ${initialY}vh)`, 
              pointerEvents: 'none' 
            };
          } else if (column === 'center') {
            return { 
              opacity: Math.max(0, 1 - easedFade), 
              transform: `translateY(${initialY - easedFade * 100}vh)`, 
              pointerEvents: 'none' 
            };
          } else if (column === 'right') {
            return { 
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* REMOVED: {!skipAnimation && ( */}
      <div ref={containerRef} className="relative min-h-[600vh] bg-gray-50">
      ...
      <div ref={flagshipRef} className="min-h-screen flex flex-col justify-center bg-white px-8 py-16">
        <h2 className="text-7xl font-bold text-gray-800 text-center mb-16">Our flagship Events</h2>
        <div className="flex justify-center gap-16">
          {flagshipEvents.map((event, index) => (
            <div key={event.id} className="flex flex-col items-center">
              <div className="relative w-80 h-96 rounded-lg shadow-xl overflow-hidden mb-6">
                <img
                  src={event.imgSrc}
                  alt={event.title}
                  className="w-full h-full object-cover"
